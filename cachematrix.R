...
# This function uses the object returned by makeCacheMatrix() to test if 
...
cacheSolve <- function(m,...) {   # m is return value from makeCacheMatrix()
...
# construct a matrix and call makeCacheMatrix()  for testing
vec <- rpois(1000000, 50)
test_matrix <- matrix(vec, nrow = 1000, ncol = 1000)
rm(vec)
cache_store <- makeCacheMatrix(test_matrix)

# compute system time to confirm inverse is being retrieved
# and calculated appropriately

inverse1 <- system.time(cacheSolve(cache_store))
inverse2 <- system.time(cacheSolve(cache_store))
